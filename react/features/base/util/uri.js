 ...
 * incompatible with Meet on the client and/or server sides.
 *
...
export function parseStandardURIString(str: string) {
    ...
    // whitespace. Anyway, the Meet app is not known to utilize unencoded
...
/**
 * Parses a specific URI which (supposedly) references a Meet resource
 * (location).
 *
 * @param {(string|undefined)} uri - The URI to parse which (supposedly)
 * references a Meet resource (location).
 * @public
 * @returns {{
 *     contextRoot: string,
 *     hash: string,
...
export function parseURIString(uri: ?string) {
    ...
    // Add the properties that are specific to a Meet resource (location)
    ...
    // XXX While the components/segments of pathname are URI encoded, Meet
